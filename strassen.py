from math import ceil,log#импорт необходимых модулей
import numpy as np
import random

random.seed(0)

def createRandomMatrix(n):#создание матриц со случайными элементами
    maxVal=1000
    matrix=[]
    for i in range(n):
...
def saveMatrix(A,B,filename):#сохранение созданных матриц в файл .txt
    f=open(filename,'w')
...
n=int(input("n="))#размерность квадратной матрицы
...
def read(filename):#считывание матриц
...
def ikjMatrixProduct(A,B):#перемножение матриц ikj методом
    n=len(A)
...
def strassenR(A,B):#реализация метода Штрассена
    n=len(A)
    if n<=LEAF_SIZE:
...
def saveMatrix(matrixA,filename):#сохранение матриц в файл .txt
    f=open(filename,'w')
...
LEAF_SIZE=int(input("leaf size: "))#8-128
